...
describe("RangeSet", () => {
	describe("resolveMissingRanges", () => {
		test("returns the requested range when no ranges are known", ({
			expect,
		}) => {
		...
		test("handles multiple known ranges inside requested range", ({
			expect,
		}) => {
			const known = [
	...
	describe("consolidate", () => {
		test("merge ranges that overlaps", ({ expect }) => {
			...
			const set = new RangeSet(ranges, timestamp(0));
			const merged = set.consolidate([
				{ from: timestamp(7), to: timestamp(15) },
			]);
		...
		test("consolidate handle parameters not sorted", ({ expect }) => {
			const set = new RangeSet(
				[
					{ from: timestamp(0), to: timestamp(10) },
					{ from: timestamp(20), to: timestamp(100) },
				],
				timestamp(0),
			);
			expect(
				set.consolidate([
					{ from: timestamp(20), to: timestamp(50) },
					{ from: timestamp(10), to: timestamp(15) },
				]),
			).toEqual([
				{ from: timestamp(0), to: timestamp(15) },
				{ from: timestamp(20), to: timestamp(100) },
			]);
		});
	});
